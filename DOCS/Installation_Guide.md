...
✅ Amber Electric account (or other dynamic pricing retailer)ie LocalVolts
...
Grid feed in price (from Amber integration)
...
sensor.amber_Feed_in_price (this is the one you'll use)




Note: sensor.amber_Feed_in_price should show current sell pricing. During negative pricing periods, it will show negative values (e.g., -0.03 or -3 depending on format).
...
Select: sensor.amber_feed_in_price
This is critical - must be the feed-in price sensor, not general price
This sensor shows negative values during negative pricing periods
